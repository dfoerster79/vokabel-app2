...
const SprachTestPage = () => {
  ...
  const startRecording = useCallback((vocab) => {
    ...
    // Diese Liste gehört ausschließlich zu diesem Recorder. Sie wird nicht
    // mit der nächsten Vokabel geteilt und kann daher nicht überschrieben werden.
    const chunks = [];
    ...
    try {
      ...
      // Ohne timeslice erzeugt Safari beim stop() einen vollständigen Container
      // mit Header. Die Chunks bleiben trotzdem lokal bei diesem Recorder.
      recorder.start();
      setIsRecording(true);
    } catch (error) {
      ...
      updateTranscription(vocab.id, {
        status: 'done',
        text: `[Aufnahmefehler: ${error.message}]`,
        correct: false,
      });
    }
  }, [updateTranscription]);
  ...
  const stopRecordingAndTranscribe = useCallback((vocab) => {
    ...
    if (!recorder || recorder.state === 'inactive') {
      if (!transcriptionsRef.current[targetVocab.id]) {
        updateTranscription(targetVocab.id, {
          status: 'done',
          text: '[Keine Aufnahme]',
          correct: false,
        });
    ...
    return new Promise(resolve => {
      recorder.onstop = async () => {
        ...
        if (blob.size > 0) {
          await transcribeBlob(blob, targetVocab);
        } else {
          updateTranscription(targetVocab.id, {
            status: 'done',
            text: '[Keine Aufnahme]',
      ...
      // Referenz auf die lokale Liste am Recorder statt in einem gemeinsamen Ref.
      // Die Liste wird nach dem Start ergänzt und beim stop() ausgewertet.
      if (!recorder.__chunks) recorder.__chunks = [];
      try {
        recorder.stop();
      } catch (_) {
        updateTranscription(targetVocab.id, {
          status: 'done',
          text: '[Aufnahme konnte nicht beendet werden]',
          correct: false,
        });
        resolve();
      }
  ...
  const handleMcAnswer = option => {
    ...
    updateTranscription(vocab.id, {
      ...
      type: 'mc',
      text: option,
      correct: option === vocab.uebersetzung,
    });
  ...
  const buildMcOptions = useCallback((list, index, waMap, pool) => {
    ...
    const distractors = ordered
      ...
      .filter((v, i, all) => v !== current.uebersetzung && all.indexOf(v) === i)
      .slice(0, 3);
  ...
  // Nach dem Wechsel der Vokabel startet die nächste Aufnahme erst jetzt,
  // also nachdem handleWeiter die vorherige Aufnahme vollständig beendet hat.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const checkDone = () => {
      const pending = vocabListRef.current.some(v => {
        const result = transcriptionsRef.current[v.id];
  ...
  const saveResults = async (finalScore, errors, timeTaken, avgTime, results) => {
    ...
    const { data: attempt, error } = await supabase
      ...
      .insert([{
        ...
        question_count: vocabListRef.current.length,
    ...
    await supabase.from('lern_attempt_fehler').insert(errors.map(v => ({
      ...
      vokabel_id: v.id,
      frage: v.original,
      gegebene_antwort: results[v.id]?.text || 'Falsch',
  ...
  const finishTest = () => {
    ...
    setTimeStats({
      ...
      average: (total / all.length).toFixed(1),
    });
    setPhase('results');
      ...
      <div style={{ maxWidth: '34rem', margin: '4rem auto', padding: '2rem', background: 'white', borderRadius: '1rem', textAlign: 'center', fontFamily: 'sans-serif', boxShadow: '0 10px 15px -3px rgba(0,0,0,0.1)' }}>
        ...
        <div style={{ maxHeight: 300, overflowY: 'auto', textAlign: 'left' }}>
          {entries.map(({ vocab, result }) => (
            <div key={vocab.id} style={{ display: 'flex', justifyContent: 'space-between', gap: 8, padding: '0.45rem 0', borderBottom: '1px solid #f3f4f6' }}>
              <span>{vocab.original}</span>
              <span style={{ color: statusColor(result) }}>{statusLabel(result)}</span>
            </div>
          ))}
        </div>
      </div>
  ...
  if (phase === 'results') return (
    <div style={{ maxWidth: '32rem', margin: '4rem auto', padding: '2rem', background: 'white', borderRadius: '1rem', textAlign: 'center', fontFamily: 'sans-serif', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.1)' }}>
      <h2>Test beendet! 🎉</h2>
      <p style={{ fontSize: '3rem', fontWeight: 'bold', color: '#0f5156' }}>{score} / {vocabList.length}</p>
      {fehlerListe.length > 0 && (
        <div style={{ textAlign: 'left', background: '#fef2f2', padding: '1rem', borderRadius: '0.75rem' }}>
          <h3 style={{ color: '#991b1b' }}>Deine Fehler:</h3>
          {fehlerListe.map(v => (
            <div key={v.id} style={{ marginBottom: '0.8rem' }}>
              <strong>{v.original}</strong><br />
              <span style={{ color: '#166534' }}>Richtig: {v.uebersetzung}</span><br />
              <span style={{ color: '#991b1b' }}>Du sagtest: {transcriptions[v.id]?.text || '[Nichts]'}</span>
            </div>
          ))}
        </div>
      )}
      <div style={{ display: 'flex', gap: '1rem', margin: '1.5rem 0' }}>
        <div style={{ flex: 1, background: '#f3f4f6', padding: '1rem', borderRadius: '0.75rem' }}>Gesamtzeit<br /><strong>{timeStats.total} s</strong></div>
        <div style={{ flex: 1, background: '#f3f4f6', padding: '1rem', borderRadius: '0.75rem' }}>Ø pro Wort<br /><strong>{timeStats.average} s</strong></div>
      </div>
      <button onClick={() => navigate('/lernen')} style={{ width: '100%', background: '#0f5156', color: 'white', padding: '1rem', borderRadius: '0.75rem', border: 'none', cursor: 'pointer', fontWeight: 'bold' }}>
  ...
  return (
    <div style={{ maxWidth: '42rem', margin: '2rem auto 5rem', padding: '0 1rem', fontFamily: 'sans-serif' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
        <span>Frage {currentIndex + 1} von {vocabList.length}</span>
        <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
          <span>⏱ {fmt(elapsed)}</span>
          <button onClick={() => setShowDebug(value => !value)} style={{ border: '1px solid #e5e7eb', borderRadius: '9999px', padding: '0.25rem 0.6rem', cursor: 'pointer' }}>
            🤖 {doneTxCount}/{currentIndex + 1}
          </button>
          <button onClick={abortTest} style={{ background: 'none', border: '1px solid #e5e7eb', borderRadius: '0.5rem', padding: '0.25rem 0.6rem', cursor: 'pointer' }}>✕ Abbruch</button>
        </div>
      </div>

      {showDebug && (
        <div style={{ background: '#1e293b', color: '#e2e8f0', borderRadius: '0.75rem', padding: '1rem', marginBottom: '1rem', fontSize: '0.8rem', fontFamily: 'monospace' }}>
          {vocabList.slice(0, currentIndex + 1).map(v => <div key={v.id}>{v.original}: <span style={{ color: statusColor(transcriptions[v.id]) }}>{statusLabel(transcriptions[v.id])}</span></div>)}
        </div>
      )}

      <div style={{ height: 6, background: '#e5e7eb', borderRadius: 99, marginBottom: '1.5rem', overflow: 'hidden' }}>
        <div style={{ height: '100%', width: `${progress}%`, background: '#0f5156', borderRadius: 99 }} />
      </div>
      ...
      <div style={{ background: 'white', borderRadius: '1.25rem', padding: '2.5rem 2rem', textAlign: 'center', boxShadow: '0 10px 15px -3px rgba(0,0,0,0.07)', marginBottom: '1.5rem', border: '1px solid #e5e7eb' }}>
        {wortartMap[current.id] && <div style={{ color: '#6b7280', marginBottom: '1rem' }}>{WORTART_LABELS[wortartMap[current.id]] || wortartMap[current.id]}</div>}
        <div style={{ fontSize: '2.5rem', fontWeight: 700 }}>{current.original}</div>
        {current.beispiel && <div style={{ marginTop: '1rem', color: '#6b7280', fontStyle: 'italic' }}>{current.beispiel}</div>}
      </div>

      {mode === 'speech' && !micReady && (
        <div style={{ textAlign: 'center', padding: '2rem', background: '#f0fdfa', borderRadius: '1rem', marginBottom: '1rem' }}>
          <div style={{ fontSize: '3rem' }}>🎙️</div>
          <p>Bitte erlaube den Mikrofon-Zugriff für den Sprachtest.</p>
          {micError && <p style={{ color: '#dc2626' }}>{micError}</p>}
          <button onClick={requestMic} style={{ background: '#0f5156', color: 'white', padding: '0.75rem 2rem', borderRadius: '0.75rem', border: 'none', cursor: 'pointer', fontWeight: 600 }}>Mikrofon erlauben</button>
        </div>
      )}

      {mode === 'speech' && micReady && (
        <div style={{ textAlign: 'center', marginBottom: '1.5rem', color: isRecording ? '#dc2626' : '#6b7280' }}>
      ...
      {showMc && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem', marginBottom: '1.5rem' }}>
          {mcOptions.map(option => <button key={option} onClick={() => handleMcAnswer(option)} style={{ background: 'white', border: '2px solid #e5e7eb', borderRadius: '0.75rem', padding: '0.85rem', cursor: 'pointer' }}>{option}</button>)}
        </div>
      )}

      {!showMc && micReady && (
        <button onClick={handleWeiter} style={{ width: '100%', background: '#0f5156', color: 'white', fontSize: '1.25rem', fontWeight: 700, padding: '1rem', borderRadius: '0.75rem', border: 'none', cursor: 'pointer' }}>
