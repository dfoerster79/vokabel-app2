...
export default function ProfilPage() {
  ...
  useEffect(() => {
    ...
    if (isFirstBundeslandLoad.current) {
      isFirstBundeslandLoad.current = false
      if (pendingOrtRef.current) {
        setOrtInput(pendingOrtRef.current)
        setOrtGewaehlt(pendingOrtRef.current)
        pendingOrtRef.current = ''
      }
      return
    }
    // Manueller Bundesland-Wechsel
    setOrtInput('')
    setOrtGewaehlt('')
    setOrtSuggestions([])
    setSchulen([])
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              <div>
                ...
                <input className="input" type="text" value={vorname}
                  onChange={e => setVorname(e.target.value)} placeholder="Vorname"
                  style={{ width: '100%' }} />
              </div>
              <div>
                <label style={labelStyle}>Nachname</label>
                <input className="input" type="text" value={nachname}
                  onChange={e => setNachname(e.target.value)} placeholder="Nachname"
                  style={{ width: '100%' }} />
              </div>
            </div>
